					...
					active = {
						add: function() { //获取选中数目
							layer.open({
								...
								fixed: false,
								scrollbar: true,
								content: CTX_PATH + "/page/addInterfaceUserInfo.jsp",
								btn: ['提交', '关闭'],
								btnAlign: 'c',
						...
						edit: function() { //获取选中数据
							...
							layer.open({
								...
								fixed: false,
								scrollbar: true,
								content: CTX_PATH + "/page/editInterfaceUserInfo.jsp?id=" + data[0].id,
								btn: ['提交', '关闭'],
								btnAlign: 'c',
								yes: function(index, layero) {
									var iframeWin = window[layero.find('iframe')[0]['name']]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method(); 
						...
						addcount: function() { //验证是否全选
							...
							layer.msg('是否为用户:'+data[0].name+'增加查询次数', {
						        time: 0, //20s后自动关闭
						        btn: ['确定', '取消'],
						        yes: function(index, layero) {
						        	layer.closeAll();
						        	layer.open({
										...
										fixed: false,
										scrollbar: true,
										content: CTX_PATH + "/page/addInterfaceUserCount.jsp?user_key=" + data[0].user_key,
										btn: ['提交', '关闭'],
										btnAlign: 'c',
										yes: function(index, layero) {
											var iframeWin = window[layero.find('iframe')[0]['name']]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method(); 
